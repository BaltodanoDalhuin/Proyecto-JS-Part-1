document.addEventListener('DOMContentLoaded', () => {
    ...
    function editTask(task, li) {
        ...
        const newDifficulty = prompt('Nueva dificultad de la tarea:', task.Dificil);
    ...
    function updateLocalStorage(key, item) {
        ...
        items = items.map(i => (i.title === item.title && i.difficulty === item.difficulty) ? item : i);
        localStorage.setItem(key, JSON.stringify(items));
    }

    function removeFromLocalStorage(key, item) {
        let items = JSON.parse(localStorage.getItem(key));
        items = items.filter(i => i.title !== item.title || i.difficulty !== item.difficulty);
        localStorage.setItem(key, JSON.stringify(items));
    }
